...
def vectoroutput_vanilla(inputs, outputs, raw_seq, n_steps_in, n_steps_out, batch_size, num_epochs):
...
def vectoroutput_stacked(inputs, outputs, raw_seq, n_steps_in, n_steps_out, batch_size, num_epochs):
  # reshape from [samples, timesteps] into [samples, timesteps, features]
  n_features = 1
  inputs = inputs.reshape((inputs.shape[0], inputs.shape[1], n_features))
  # define model
...
def vectoroutput_bidirectional(inputs, outputs, raw_seq, n_steps_in, n_steps_out, batch_size, num_epochs):
  # reshape from [samples, timesteps] into [samples, timesteps, features]
  n_features = 1
  inputs = inputs.reshape((inputs.shape[0], inputs.shape[1], n_features))
  # define model
...
def encoderdecoder_vanilla(inputs, outputs, raw_seq, n_steps_in, n_steps_out, batch_size, num_epochs):
  # reshape from [samples, timesteps] into [samples, timesteps, features]
  n_features = 1
  inputs = inputs.reshape((inputs.shape[0], inputs.shape[1], n_features))
  ...
  model.add(LSTM(batch_size, activation='relu', return_sequences=True))
